...
class BlogExtractSpider(scrapy.Spider):
    ...
    allowed_domains = ["bemorewithless.com"]
    start_urls = ['https://bemorewithless.com/archives/']

    def parse(self, response):
        print 'next_page_url ', response.url
        blogs = response.xpath('//div[@class="archive-list"]//a')
    ...
    def parse_sub_blog(self, response):
        ...
        comments_count = response.xpath('//span[@class="entry-comments-link"]/a/text()').re(r'\s*(\S+)\s*Comment')
        ...
        ignore_tags = ["share-before_entry_header share-filled share-small"]
        word_count = 0
