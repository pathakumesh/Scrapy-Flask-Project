...
class BlogExtractSpider(scrapy.Spider):
    ...
    allowed_domains = ["erinblaskie.com"]
    start_urls = ['http://erinblaskie.com/blog']
    ...
    def parse_sub_blog(self, response):
        ...
        item['date'] = response.xpath('//p[@class="entry-meta"]/time/text()').extract_first()
        item['author'] = response.xpath('//p[@class="entry-meta"]//span[@class="entry-author-name"]/text()').extract_first()
        ...
        continue_tags = ["share-before share-filled share-small"]
        word_count = 0
...
def count_after_replacing_special_chars(string):
    ...
    for _line in updated_line:
        if len(_line) > 0:
            ...
            if 'jQuery(document).ready' in _line:
                break
