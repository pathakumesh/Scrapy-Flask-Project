...
class BlogExtractSpider(scrapy.Spider):
    ...
    allowed_domains = ["spirituallivingforbusypeople.com","disqus.com"]
    start_urls = ['https://www.spirituallivingforbusypeople.com/blog']

    def parse(self, response):
        blog_posts = response.xpath('//div[contains(@class, "blog-posts")]')
        print 'page_url ', response.url
        for sub_blog in  blog_posts.xpath('article[@id]'):
            ...
            #Get Title, Link, Date
            title = sub_blog.xpath('header/h1[@class="entry-title"]/a/text()').extract_first().strip()
            link = sub_blog.xpath('header/h1[@class="entry-title"]/a/@href').extract_first()
            date = sub_blog.xpath('footer/span[@class="post-date"]/a/time/text()').extract_first().strip()
            author = sub_blog.xpath('footer/span[@class="by-author"]/span/a/text()').extract_first().strip()
        ...
        next_page = response.xpath('//div[@class = "nav-previous"]/a')
        if next_page:
    ...
    def parse_sub_blog(self, response):
        ...
        main_block = response.xpath('//div[@class="entry-content clearfix"]/*')
        
        ignore_blocks = ["tve-leads-post-footer tve-tl-anim tve-leads-track-post_footer-15 tl-anim-instant tve-leads-triggered"]
        word_count = 0
...
def get_comments_url(link, title, post_id):
    ...
    params =  {
        ...
        "f": "wwwcylongeorgecom",
        "t_u": link,
        "t_i": "%s https://spirituallivingforbusypeople.com/?p=%s" % (post_id, post_id),
        "t_e": title,
        "t_d": title + " - Spiritual Living For Busy People",
        "t_t": title,
        "s_o": "default"
    }
    try:
